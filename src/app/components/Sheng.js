    ...
    return (
        <section className="flex absolute p-8 gap-4 w-screen">
                ...
                <div className="flex absolute right-0 pr-8 gap-4 box-border">
                    ...
                    <Link  data-active={checkPath("/work","active")} href="/work" onMouseEnter={()=>animate(span2Ref)}  onMouseLeave={()=>handleLeave(span2Ref)} className={`${checkPath("/work","text")} relative cursor-pointer uppercase hover:opacity-75 transition delay-150 duration-700`}>
                        work
                        <span ref={span2Ref} className={`${checkPath("/work","bar")} absolute left-0 bottom-0 h-[2px] w-[0%]`}></span>
                    </Link>
                </div>
        </section>
    )
