...
export default function Home() {
  ...
  const mouseLeave=()=>{
    const tl = gsap.timeline();
    if(!animEnded){
      tl.to(leftRef.current,{duration:0.5, height:"0%"}).to(topRef.current,{duration:0.5,width:"0%"}).to(rightRef.current,{duration:0.5,height:"0%"}).to(bottomRef.current,{duration:0.5,width:"0%",onComplete:()=>{
        setAnimEnded(true)
      }})
    }
  }
    ...
    <div className="grid relative h-[100vh] w-[100vw] overflow-hidden font-[family-name:var(--font-geist-sans)]">
      {!completed && !animEnded?<section ref={blockerRef} className="absolute grid bg-white w-[100%] h-[100%] z-999">
          <section className="relative place-self-center">
              ...
              <div ref={rightRef} className="bg-black w-[1px] h-[0%] scale-110 translate-x-[15px] right-0 absolute"></div>
